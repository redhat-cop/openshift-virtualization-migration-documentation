...
First, specify the URL that should be used to clone the source code within Ansible Automation Platform. This can be found by navigating to the Ansible for OpenShift Virtualization Migration Ansible Content Collection link:https://github.com/redhat-cop/openshift_virtualization_migration[GitHub repository], clicking the green *Code* button, and selecting the URL for the retrieval method being used (PAT-based authentication should use the *HTTPS* option).
...
Tooling is available in the link:https://github.com/redhat-cop/openshift-virtualization-vmware-vddk[openshift-virtualization-vmware-vddk repository] to help produce a VDDK image using customer/end user provided licensed VMware components.
====
